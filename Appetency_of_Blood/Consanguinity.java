...
public class Consanguinity{
	...
	public static void addParents(Human h, int n){
		if (n != 0){
			list.add(h);
			addParents(h.father, n-1);
			addParents(h.mother, n-1);
		}
	}
	...
	public static String buildFamilyTree(Human h){
		...
		int y = 460;
		s += "<text x='50%' y='"+(y)+"' text-anchor='middle'>"+h.getName().getFull()+"</text>";
		s += buildLineage(h.father, y-15, 2, 33);
		s += buildLineage(h.mother, y-15, 0, 66);
		s += buildLineage(h.father.mother, y-65, 0, 66);
		s += buildLineage(h.father.father.mother, y-115, 0, 66);
	...
	public static String buildLineage(Human h, int y, int n, int x){
		...
		s += 	"<text x='"+x+"%' y='"+(y-30)+"' text-anchor='middle'>"+h.getName().getFull()+"</text>";
		if (n > 0){			return s+buildLineage(h.father, y-50, n-1, x);
		} else {			return s; }
	}
	...
	public static String getPatrilinearCollaretal(Human from, Human to){
		...
		if (v == 0){
			...
			return Main.getOrder(v-1)+" cousin";
		}
	}
	...
	public static String getPaternalAvunculate(Human from, Human to){
		...
		switch(v){
			...
			default:
				return Main.getOrder(v-1)+" cousin once removed";
		}
	}
	...
	public static String getPaternalGrandparent(Human from, Human to){
		...
		switch(v){
			...
			default:
				return Main.getOrder(v-1)+" cousin twice removed";
		}
	}
	...
	public static String getPatGreatgrandparent(Human from, Human to){
		...
		switch(v){
			...
			default:
				return Main.getOrder(v-1)+" cousin thrice removed";
		}
	}
	...
	public static void printDes(Human h, int i){
		...
		System.out.println(h+" "+h.getHouse()+" "+h.getLifespan());
		if (h.isAdult()){
			for(Human x: h.sons){
				printDes(x, i+1);
			}
