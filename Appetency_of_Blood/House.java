...
public class House {
    ...
    public House(Human head){
        id++;
		this.active = 							true;
		this.founding = 						(Calendar) Main.date.clone();
		this.head = 							head;
		...
		this.naming	=							Main.randint(6);
		int n = 								fetchName();
		this.name = 							names[n];
		this.nameNum =							n;
		Main.house.put(House.id, this);
		list.add(this);
    }
		...
		*/
		...
		if (heir != h){
			if (heir.house == this){
				...
				if (this.head.title == null){
					this.head.rename(Title.LORD);
					if (this.head.spouse != null){ this.head.spouse.rename(Title.LADY); }
				}
				return;
			}
		}
		...
		if (this.findNextHouse()){
			...
			Main.print("The senior line of "+this.getFullName()+" went extinct, but was succeeded by "+nextHouse.getName());
		} else{
			Main.print(this.getName()+" went extinct");
		}
	...
	public int getSeniorBranchNum(){
		...
		try{
			...
			h = this.getHead();
			System.out.print(h.getHouse()==p.getHouse());
...
//Name methods
	...
	public String getMemberNameM(Human c){
		switch(this.naming){
			...
			case 2:
				return this.doFlatNamingM(c);				//flat random naming
			case 3:
	...
	public String getMemberNameF(Human c){
		switch(this.naming){
			...
			case 2:
				return this.doFlatNamingF(c);				//flat random naming
			case 3:
	...
	public String pickRandomNameF(){
		...
		if (newN.charAt(newN.length()-1) == 's'){
	...
	public List<String> getUsableNamesM(Human h){
		...
		for(String x: houN){
			if(!h.getLivingSonsNames().contains(x)){
				pot.add(x);
			}
		}
		return pot;
	...
	public List<String> getUsableNamesF(Human h){
		...
		List<String> pot = 		new ArrayList<>();
		for(String x: houN){
			if(!h.getLivingDaughtersNames().contains(x)){
				pot.add(x);
			}
		}
		return pot;
...
//House prince methods
	...
	public void makePrinces(){
		for(Human x: this.getPatriarch().getLivingSons()){
			this.addPrince(x);
		}
	}

	public static int fetchName(){
		int n = Main.randint(namesN.size());
		int s = namesN.get(n);
...
//Micro methods
	...
	public static House getHouse(int id){			return Main.house.get(id);	}
	...
	private static String[] names = new String[220];
